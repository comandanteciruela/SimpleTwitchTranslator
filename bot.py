    ...
    try:
        ...
        for var, name in zip([BOT_OAUTH_TOKEN, BOT_CLIENT_ID], ["BOT_OAUTH_TOKEN", "BOT_CLIENT_ID"]):
            if not is_valid(var):
                print(f"{DEBUG_BOLD_ORANGE}{ERROR_BOLD_RED}{name} must be a string with more than 18 alphanumeric characters.")
                exit(1)

        if not (isinstance(CHANNEL_NATIVE_LANG, str) and len(CHANNEL_NATIVE_LANG) == 2):
            print(f"{DEBUG_BOLD_ORANGE}{ERROR_BOLD_RED}CHANNEL_NATIVE_LANG must be a string with exactly 2 characters. Examples: es, en, ja, ru")
            exit(1)

        if not (isinstance(TRANSLATE_TO_LANG, str) and len(TRANSLATE_TO_LANG) == 2):
            print(f"{DEBUG_BOLD_ORANGE}{ERROR_BOLD_RED}TRANSLATE_TO_LANG must be a string with exactly 2 characters. Examples: es, en, ja, ru")
            exit(1)
    ...
    class Bot(commands.Bot):
        ...
        async def send_random_messages(self):
            ...
            if not (isinstance(interval, (int, float)) and interval > 0):
                print(f"{DEBUG_BOLD_ORANGE}{ERROR_BOLD_RED}Invalid RANDOM_MESSAGES_INTERVAL; defaulting to {DEFAULT_RANDOM_MESSAGES_INTERVAL} seconds.")
                ...
                async with ClientSession() as session:
                    async with session.get(
                        ...
                        if response.status == 200:
                            print(f"{DEBUG_BOLD_ORANGE}{OK_BOLD_GREEN}Successful connection.")
                            data = await response.json()
                            if data.get("data"):
                                return True, data["data"][0]
                            else:
                                print(f"{DEBUG_BOLD_ORANGE}{ERROR_BOLD_RED}No user data found.")
                                return False, None
                        else:
                            print(f"{DEBUG_BOLD_ORANGE}{ERROR_BOLD_RED}Connection error: {response.status}")
                            return False, None
            except Exception as e:
        ...
        async def handle_translation(self, message):
            try:
                ...
                if isinstance(detected_lang, list) and len(detected_lang) == 2:
                    ...
                    if translated_text:
                        print(f"{DEBUG_BOLD_ORANGE}{OK_BOLD_GREEN}Message sent: {formatted_message}")
                        await sleep(0.35)
                        ...
                        print(f"{DEBUG_BOLD_ORANGE}{ERROR_BOLD_RED}Could not translate the message.")
                else:
                    print(f"{DEBUG_BOLD_ORANGE}{ERROR_BOLD_RED}Detection response is not valid.")
            except Exception as e:
                print(f"{DEBUG_BOLD_ORANGE}{ERROR_BOLD_RED}Error processing the message: {e}")
